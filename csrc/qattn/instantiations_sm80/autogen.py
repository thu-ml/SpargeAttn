...
for hd, qkg, pv_mode, dtype_out, causal, ret_pv_count, ret_lse in product(
        head_dims, qk_quant_grans, pv_threshold_modes, dtypes_out, is_causals, return_pv_counts, return_lses):
    ...
    filename = (
        ...
        f"_pvth{pv_mode}_o{dtype_out}_causal{bool_to_int(causal)}_retpvth{bool_to_int(ret_pv_count)}_retlse{bool_to_int(ret_lse)}.cu"
    )
    ...
    instantiation = (
        ...
        f"{str(causal).lower()}, {str(ret_pv_count).lower()}, {str(ret_lse).lower()}"
